...
def population_csv_for(report_path: Path) -> Path:
    return report_path.with_suffix(".population.csv")


def popwiring_csv_for(report_path: Path) -> Path:
    return report_path.with_suffix(".popwiring.csv")


def parse_population(csv_path: Path) -> list[dict]:
    rows = []
            ...
            rows.append(
                {
                    ...
                    "en": float(row["energy"]),
                    "dmg": float(row.get("damage", 0.0) or 0.0),
                }
            )
    return rows
...
def build_html(meta: dict, units: list[dict], wiring: dict, lineages: list[dict],
               timeseries: list[dict], population: list[dict], popwiring: dict,
               report_name: str) -> str:
    payload = json.dumps({"meta": meta, "units": units, "wiring": wiring,
                          "lineages": lineages, "timeseries": timeseries,
                          "population": population, "popwiring": popwiring})
...
<div id="linevo" style="display:none">
<div class="sectit">Lineage evolution — founder vs evolved descendant</div>
<div class="grid">
  <div class="panel" style="grid-column: 1 / -1;"><h2>Watch one lineage evolve its circuit</h2>
    <div class="desc">Pick a surviving lineage and one of its living members. <b class="b">Left</b> = the founder's circuit (generation 0, from the seeded genome). <b class="b">Right</b> = the chosen descendant's circuit now — every difference is point mutation accumulated across the generations between them. Same layout rules as the per-unit view (depth left→right, green/red = weight sign, dashed = feedback, bright = live core).</div>
    <div style="display:flex; gap:10px 14px; align-items:center; margin-bottom:10px; flex-wrap:wrap;">
      <label>lineage <select id="lvsel"></select></label>
      <label>member <select id="lvmem"></select></label>
      <label style="font-size:12px; color:#7d8590;"><input type="checkbox" id="lvjunk"> show junk wiring</label>
    </div>
    <div style="display:grid; grid-template-columns: 1fr 1fr; gap: 14px;">
      <div><div style="color:#7d8590; font-size:11px; margin-bottom:4px;">founder · generation 0</div>
        <svg id="lvfounder" viewBox="0 0 720 300" width="100%"></svg></div>
      <div><div style="color:#7d8590; font-size:11px; margin-bottom:4px;" id="lvmemlabel">descendant</div>
        <svg id="lvdesc" viewBox="0 0 720 300" width="100%"></svg></div>
    </div>
    <div class="verdict" id="lvinfo"></div></div>
</div>
</div>
...
const LBL={ACTION_Y:"ACT",HARVEST:"HARV",RESOURCE:"RES",SELF_ENERGY:"SELF",REPRODUCE:"REPR"};
const ORDER=["CONST","RAND","RESOURCE","SELF_ENERGY","ADD","MUL","CLAMP","DELAY","THRESH","ACTION_Y","HARVEST","REPRODUCE"];
const SENSORS=new Set(["CONST","RAND","RESOURCE","SELF_ENERGY"]), ACTIONS=new Set(["ACTION_Y","HARVEST","REPRODUCE"]);
...
// renders an edge list into one SVG by signal depth; returns derived facts.
// shared by the founder unit-chooser and the lineage-evolution comparison.
function renderCircuit(svg, all, showJunk){
  svg.innerHTML="";
  ...
  return {nodes, core, feedbackCount, fwd, maxL, edgeCount:all.length,
          meaningful:all.filter(e=>e[3]===1).length};
}

function drawWiring(){
  const u=+usel.value, all=WIRING[u]||[];
  const info=renderCircuit(document.getElementById("wiring"), all, document.getElementById("showjunk").checked);
  const ud=U.find(x=>x.unit===u)||{};
  const hasPath=[...info.fwd].some(n=>ACTIONS.has(n));
  const drivesHarvest=info.core.has("HARVEST");
  const yn=(b,t,c)=> `<b class="${b?(c||'g'):'r'}">${b?'✓':'✗'}</b> ${t}`;
  document.getElementById("uinfo").innerHTML =
    `unit ${u} · ${ud.mc} meaningful / ${all.length} total edges · ${info.nodes.size} nodes · depth ${info.maxL} · `+
    `lifespan <b>${ud.ls}</b> · ${ud.rg}` + (ud.ht>0?` · fed <b class="a">${ud.ht}</b> ticks`:` · never fed`) + ` &nbsp;|&nbsp; `+
    yn(hasPath,"sensor→action path") + ` · ` +
    yn(info.feedbackCount>0,"feedback / memory","b") + ` · ` +
...
// ---- lineage evolution: founder vs descendant circuits ----
(function(){
  const POP = DATA.population || [], PW = DATA.popwiring || {};
  if (!POP.length || !Object.keys(PW).length) return;
  document.getElementById("linevo").style.display = "";
  const lvsel=document.getElementById("lvsel"), lvmem=document.getElementById("lvmem"),
        lvjunk=document.getElementById("lvjunk");

  const byLin={};
  POP.forEach(p=>{ (byLin[p.lin]=byLin[p.lin]||[]).push(p); });
  const lins=Object.keys(byLin).map(Number).filter(l=>WIRING[l])
    .sort((a,b)=> Math.max(...byLin[b].map(p=>p.gen)) - Math.max(...byLin[a].map(p=>p.gen)));
  if(!lins.length) return;
  lins.forEach(l=>{ const o=document.createElement("option"); o.value=l;
    const g=Math.max(...byLin[l].map(p=>p.gen));
    o.textContent=`lineage ${l} · ${byLin[l].length} alive · gen≤${g}`; lvsel.appendChild(o); });

  function fillMembers(){
    const l=+lvsel.value; lvmem.innerHTML="";
    [...byLin[l]].sort((a,b)=>b.gen-a.gen).forEach(p=>{
      const o=document.createElement("option"); o.value=p.slot;
      o.textContent=`gen ${p.gen} · slot ${p.slot}`; lvmem.appendChild(o); });
  }
  function draw(){
    const l=+lvsel.value, slot=+lvmem.value, junk=lvjunk.checked;
    const fInfo=renderCircuit(document.getElementById("lvfounder"), WIRING[l]||[], junk);
    const dInfo=renderCircuit(document.getElementById("lvdesc"), PW[slot]||[], junk);
    const mem=byLin[l].find(p=>p.slot===slot)||{};
    document.getElementById("lvmemlabel").textContent=`descendant · generation ${mem.gen} (slot ${slot})`;
    const dm=e=> `${e.meaningful} meaningful / ${e.edgeCount} edges · ${e.nodes.size} nodes · depth ${e.maxL}`+
      (e.core.has("HARVEST")?` · <b class="a">eats</b>`:``)+(e.core.has("REPRODUCE")?` · <b class="p">breeds</b>`:``);
    document.getElementById("lvinfo").innerHTML=
      `lineage <b>${l}</b> · founder gen 0 → member gen <b>${mem.gen}</b> · energy <b>${fmt(mem.en)}</b> · damage <b>${fmt(mem.dmg)}</b><br>`+
      `founder: ${dm(fInfo)}<br>descendant: ${dm(dInfo)}`;
  }
  lvsel.addEventListener("change",()=>{ fillMembers(); draw(); });
  lvmem.addEventListener("change",draw);
  lvjunk.addEventListener("change",draw);
  fillMembers(); draw();
})();
</script>
</body>
...
def main(argv: list[str]) -> int:
    ...
    population = parse_population(population_csv_for(report_path))
    popwiring = parse_wiring(popwiring_csv_for(report_path))
    out_path.write_text(build_html(meta, units, wiring, lineages, timeseries,
                                   population, popwiring, report_path.name))
    print(f"wrote {out_path} ({len(units)} units, wiring for {len(wiring)}, "
          f"{len(lineages)} lineages, {len(timeseries)} time samples, "
          f"{len(population)} alive, evolved wiring for {len(popwiring)})")
    return 0
